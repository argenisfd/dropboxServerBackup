...
db_files=[]
for db in arguments.get("databases"):
...
dropboxSync= DropboxSync(arguments.get("dropbox").get("token"))
for tar_file in compress_files:
    dropboxSync.send("temp/"+tar_file,arguments.get("dropbox").get("sql_backup_folder")+tar_file)
